...
# Function to plot the maximum real eigenvalues across a gradient of K2 and K1
...
function plot_consumption(K2_range, K1_base, u0, p)
    ...
    for K2_val in K2_range
        ...
        Ω = p.pref(eq, p, 0.0)
        consumption_C1_val = eq[5] * (Ω * p.a_PC1 * eq[2] * eq[5] / (1 + Ω * p.a_PC1 * p.h_PC1 * eq[2] + (1 - Ω) * p.a_PC2 * p.h_PC2 * eq[4]))
        consumption_C2_val = eq[5] * ((1 - Ω) * p.a_PC2 * eq[4] * eq[5] / (1 + Ω * p.a_PC1 * p.h_PC1 * eq[2] + (1 - Ω) * p.a_PC2 * p.h_PC2 * eq[4]))
